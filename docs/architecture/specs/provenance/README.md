...
The design of Nevermined using both specifications is critical because enable the
...
Relations between agents and entities are open but there some common relations that can be used as reference:
...
given usage; it is permitted for an activity to have used a same entity multiple
 times.)
...
In the following example we can see how is associated the activity `did:nv:ac1`
with the usage of the entity `did:nv:e1` in sone specific moment.
...
For describing the architecture we are gonna use as a reference a supply chain chain use case. In this scenario a company manufacturing some goods needs to deliver them to the final customer. To do that the goods need to be transported by different shipping companies until the goods are handed over to the final recipient.  
